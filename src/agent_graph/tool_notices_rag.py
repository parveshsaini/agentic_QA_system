...
class NoticesRAGTool:
    """
    A tool for retrieving relevant university notices and circulars using a 
    ...
    documents or entries from a specific collection, such as University notices.
    ...
    def __init__(self, embedding_model: str, vectordb_dir: str, k: int, collection_name: str) -> None:
        """
        Initializes the NoticesRAGTool with the necessary configuration.
...
@tool
def lookup_notices(query: str) -> str:
    """Lookup NSUT University notices and find answers for queries regarding University events and academic details."""
    rag_tool = NoticesRAGTool(
        embedding_model=TOOLS_CFG.notices_rag_embedding_model,
        vectordb_dir=TOOLS_CFG.notices_rag_vectordb_directory,
        k=TOOLS_CFG.notices_rag_k,
        collection_name=TOOLS_CFG.notices_rag_collection_name)
