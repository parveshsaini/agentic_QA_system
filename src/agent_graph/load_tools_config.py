...
class LoadToolsConfig:
    ...
    def __init__(self) -> None:
        ...
        # Notices RAG configs
        self.notices_rag_llm = app_config["notices_rag"]["llm"]
        self.notices_rag_llm_temperature = float(
            app_config["notices_rag"]["llm_temperature"])
        self.notices_rag_embedding_model = app_config["notices_rag"]["embedding_model"]
        self.notices_rag_vectordb_directory = str(here(
            app_config["notices_rag"]["vectordb"]))  # needs to be strin for summation in chromadb backend: self._settings.require("persist_directory") + "/chroma.sqlite3"
        self.notices_rag_unstructured_docs_directory = str(here(
            app_config["notices_rag"]["unstructured_docs"]))
        self.notices_rag_k = app_config["notices_rag"]["k"]
        self.notices_rag_chunk_size = app_config["notices_rag"]["chunk_size"]
        self.notices_rag_chunk_overlap = app_config["notices_rag"]["chunk_overlap"]
        self.notices_rag_collection_name = app_config["notices_rag"]["collection_name"]
        ...
        # Projects SQL Agent configs
        self.projects_sqldb_directory = str(here(
            app_config["projects_sqlagent_configs"]["projects_sqldb_dir"]))
        self.projects_sqlagent_llm = app_config["projects_sqlagent_configs"]["llm"]
        self.projects_sqlagent_llm_temperature = float(
            app_config["projects_sqlagent_configs"]["llm_temperature"])
